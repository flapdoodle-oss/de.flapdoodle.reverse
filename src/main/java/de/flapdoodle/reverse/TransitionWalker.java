...
public class TransitionWalker {
	...
	public <D> ReachedState<D> init(StateID<D> destination, InitListener...listener) {
		return init(new LinkedHashMap<>(), destination, Collections.unmodifiableList(Arrays.asList(listener)));
	}

	private <D> ReachedState<D> init(Map<StateID<?>, State<?>> currentStateMap, StateID<D> dest, List<InitListener> initListener) {
...
//			});
	...
	public static class ReachedState<D> implements AutoCloseable {
		...
		public <T> ReachedState<T> init(StateID<T> destination) {
			return parent.init(stateMap, destination, initListener);
		}
